...
namespace stdext
{
    ...
    struct is_arithmetic_container<T,
        std::void_t< typename std::remove_cvref_t<T>::value_type >> : std::conjunction <  is_container< std::remove_cvref_t<T> >,
                                                        std::negation< is_string<T> >,
        std::is_arithmetic< typename std::remove_cvref_t<T>::value_type > > {};
    template<typename T>
